...
public class CvsExport {
	...
	public void save(String path, Collection<IExtensionPoint> extPts,
			Collection<CustomExtensionPoint> cExtPts,
			Collection<IPluginModelBase> plugins) throws IOException {
		...
		for (IExtensionPoint extPt : extPts) {
			...
			// for each plugin
			for (IPluginModelBase plugin : plugins) {
				int nb = E4MigrationRegistry.getDefault().getInstanceNumber(
						extPt, plugin);
			...
			ISchemaElement[] ses = schema
					.getCandidateChildren(extensionElement);
			for (ISchemaElement se : ses) {
				buf = new StringBuffer();
				buf.append(uniqueIdentifier).append(",")
						.append(se.getSchema().getPointId());
				for (IPluginModelBase plugin : plugins) {
					int nb = E4MigrationRegistry.getDefault()
							.getInstanceNumber(se, plugin);
					buf.append(",").append(nb);
				}
				buffer.write(buf.toString());
				buffer.newLine();
	...
	private ISchema getSchema(String uniqueIdentifier) {
		ISchema s = PDECore.getDefault().getSchemaRegistry()
				.getSchema(uniqueIdentifier);
		if (s == null) {
